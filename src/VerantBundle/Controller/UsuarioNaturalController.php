...
use VerantBundle\Form\UsuarioNaturalType;
...
class UsuarioNaturalController extends Controller {
    ...
    public function indexAction(Request $request)
    {
        ...
        if ($session->get('login') == true)://verifico si el usuario esta logueado
            ...
            $dql = "SELECT u FROM VerantBundle:Usuario u WHERE u.tipo = 'USU_NAT'";
            ...
            return $this->render('usuarionatural/index.html.twig', array(
...
//                $imagenUsu = $form->get('imagenUsu')->getData(); //rescato la imagen del usuario
                $imagenCi = $form->get('imagenCi')->getData(); // rescato la imagen del CI
                //formateo de rut
...
//                }
                ...
                if (($imagenCi instanceof UploadedFile) && $imagenCi->getError() == 0) {

                    $tipo = $imagenCi->getMimeType();
                    $file = file_get_contents($imagenCi->getPathname());
                    $base64 = base64_encode($file);
                    $usuario->setImagenCiTipo($tipo);
                    $usuario->setImagenCi($base64);
...
//                echo "<img src='data:".$imagen->getMimeType().";base64,".$base64."' height='400'/>";
                ...
                return $this->redirectToRoute('usuario_natural_index', array('id' => $usuario->getId()));
            }

            return $this->render('usuarionatural/new.html.twig', array(
                        'usuario' => $usuario,
...
//            exit();
            ...
            return $this->render('usuarionatural/show.html.twig', array(
                        'usuario' => $usuario,
    ...
    public function editAction(Request $request, Usuario $usuario)
    {
        ...
        if ($session->get('login') == true)://verifico si el usuario esta logueado
            ...
            $editForm = $this->createForm('VerantBundle\Form\UsuarioNaturalType', $usuario); // creo el formulario
//            $editForm->setRut($rut . '-' . $dv);
            ...
            if ($editForm->isSubmitted() && $editForm->isValid()) { //solo si se envía un formulario
                $imagenCi = $editForm->get('imagenCi')->getData();//obtengo el archivo de CI del usuario // desde el formulario
                $terminosCondi = $editForm->get('terminos')->getData();
                ...
                if (($imagenCi instanceof UploadedFile) && $imagenCi->getError() == 0) {
                    ...
                    $ciRecover = $this->recoverImageCi($usuario->getId());
                    
//                    print_r($ciRecover[0]);
//                    exit();
                    if($ciRecover[0]['imagenCiTipo'] != ""){
                    
                        $ciI = stream_get_contents($ciRecover[0]['imagenCi']);
                        $ciT = $ciRecover[0]['imagenCiTipo'];

                        $usuario->setImagenCiTipo($ciT);
                        $usuario->setImagenCi($ciI);
                    }
                    else{
                        $this->get('session')->getFlashBag()->add("mensaje", "Problemas con archivo de Cédula de identidad en usuario con el ID: ".$usuario->getId()); //mensaje para loguearse
                        return $this->redirectToRoute('usuario_natural_index', array('id' => $usuario->getId()));
                    }
                }
                
                if (($terminosCondi instanceof UploadedFile) && $terminosCondi->getError() == 0) {

                    $tipo = $terminosCondi->getMimeType();
                    $file = file_get_contents($terminosCondi->getPathname());
                    $base64 = base64_encode($file);
                    $usuario->setTerminosTipo($tipo);
                    $usuario->setTerminos($base64);
                } 
                else {
                    $ciRecover = $this->recoverTerminos($usuario->getId());
                    if($ciRecover[0]['terminosTipo'] != ""){
                    $ciI = stream_get_contents($ciRecover[0]['terminos']);
                    $ciT = $ciRecover[0]['terminosTipo'];
                    
                    $usuario->setTerminosTipo($ciT);
                    $usuario->setTerminos($ciI);
                    }
                    else{
                        $this->get('session')->getFlashBag()->add("mensaje", "Problemas con archivo de Términos y Condiciones en usuario con el ID: ".$usuario->getId()); //mensaje para loguearse
                        return $this->redirectToRoute('usuario_natural_index', array('id' => $usuario->getId()));
                    }
                }
            ...
            return $this->render('usuarionatural/edit.html.twig', array(
                        'usuario' => $usuario,
    ...
    public function deleteAction(Request $request, Usuario $usuario)
    {
        ...
        if ($session->get('login') == true)://verifico si el usuario esta logueado
            ...
            return $this->redirectToRoute('usuario_natural_index');
...
//        exit();
    ...
    private function recoverImageCi($id)
    {
        $em = $this->getDoctrine()->getManager();
        $query = $em->createQuery(
                        "Select u.imagenCi, u.imagenCiTipo from VerantBundle:Usuario u Where u.id = :id")->setParameter('id', $id);
...
//        exit();
    ...
    private function recoverTerminos($id)
    {
        $em = $this->getDoctrine()->getManager();
        $query = $em->createQuery(
                        "Select u.terminos, u.terminosTipo from VerantBundle:Usuario u Where u.id = :id")->setParameter('id', $id);
